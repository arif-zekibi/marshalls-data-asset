...
def createTable():
    try:
        ...
        if strTargetTableCreated == '0':
            ...
            strCreateTableQuery = strCreateTableQuery.format(TableName=strTempTableName,Columns = strTableCols)
            print(strCreateTableQuery)
            spark.sql(strCreateTableQuery)
            dfTempTable = spark.sql('SELECT * FROM '+strTempTableName)
            print('\nTable Schema',dfTempTable.schema)
            dfTempTable.write.mode('ignore').format('delta')\
                .save(strTargetTablePath)
        ...
        spark.sql('DROP TABLE if EXISTS '+strTableForSchema)
        spark.sql('DROP TABLE if EXISTS '+strTempTableName)
